...
def test_can_get_list_of_attached_projects(mock_client):
    resp = mock_client.get("/v1/projects/attached?client=1")
...
def test_can_detach_project(mock_client):
    resp = mock_client.post(
        "/v1/projects/detach?client=1", json={"url": "https://testproject.org"}
    )
